...
export class ContactoService extends GenericService<Contacto> implements GenericServiceBase<Contacto> {
    ...
    getByID(ID: number) {
        let respone: Subject<Contacto> = new Subject();
        let params = this.db.createParameter('CRM0001', 2, { V3: 0, V4: ID});
    ...
    save(item: Contacto) {
        ...
        let tParams = this.db.createParameter('CRM0001', 1, { 
            V3: item.tipoID,

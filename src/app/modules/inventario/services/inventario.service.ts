...
import { Producto, CategoriaProductoSumary, CategoriaProducto } from 'app/modules/producto/models/producto.models';
...
export class InventarioService extends GenericService<Inventario> implements GenericServiceBase<Inventario> {
  ...
  mapData(item: any){
    let cat = new CategoriaProductoSumary(item.C6);
    cat.key = item.C4;

    let prod = new Producto(item.C1);
    prod.key = item.C0;
  ...
  getInventarioProducto(productoID: number, sucursalID: number){
    this.startLoading();
    let params = this.db.createParameter('INV0002', 2, {
      'V3': productoID,
      'V4': sucursalID
    });
  ...
  getInventarioActual(sucursalID: number){
    this.startLoading();
    let params = this.db.createParameter('INV0001', 1, {
      'V4': sucursalID
    });
  ...
  realizarCorte(sucursalID: number, inventario: Inventario[]){
    ...
    this.startLoading();
    let inventario2Send = inventario.filter(inv=> inv.cantidad !== inv.cantidadFisica && inv.cantidadFisica);
    let params = this.db.createParameter('INV0001', 3, {
      'V4': sucursalID,
      'V6': this.mapInventario2Server(inventario2Send)
    });
    let wc = this.db.getData(params);
    wc.subscribe(result => {
      this.source$.next(this.mapList(result.Table));
      this.finishLoading();
    });
